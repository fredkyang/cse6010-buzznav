...
{ // Initialize graph represented by adjacency list
...
int load_node_coordinates(Graph *g, const char *filename)
{
    ...
    if (!input)
    {
        fprintf(stderr, "\033[31m:( Failed loading coordinates file '%s'\n", filename);
        return -1;
    }
    char buffer[256];
    int node_id;
    ...
    while (fscanf(input, "%d,%lf,%lf", &node_id, &lon, &lat) == 3)
    {
    ...
    fclose(input);
    fprintf(stdout, "\033[32m:) Load node coordinates successfully (using x as lon, y as lat)\n");
    return 0;
}
